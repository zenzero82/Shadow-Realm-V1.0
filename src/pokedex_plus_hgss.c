...
static void Task_LoadStatsScreen(u8 taskId)
{
    ...
    case 5:
        if (gTasks[taskId].data[1] == 0)
        {
            ...
            gTasks[taskId].data[6] = CreateMonIcon(species, SpriteCB_MonIcon, 18, 31, 4, personality, FALSE); //Create pokemon sprite
...
static void Task_LoadEvolutionScreen(u8 taskId)
{
    ...
    case 3:
        if (gTasks[taskId].data[1] == 0)
        {
            ...
            gTasks[taskId].data[4] = CreateMonIcon(species, SpriteCB_MonIcon, 18 + 32*sPokedexView->numPreEvolutions, 31, 4, personality, FALSE); //Create pokemon sprite
...
static void HandleTargetSpeciesPrintIcon(u8 taskId, u16 targetSpecies, u8 base_i, u8 iterations)
{
    ...
    if (iterations > 6) // Print icons closer to each other if there are many evolutions
        gTasks[taskId].data[4+base_i] = CreateMonIcon(targetSpecies, SpriteCB_MonIcon, 45 + 26*base_i, 31, 4, personality, FALSE);
    else
        gTasks[taskId].data[4+base_i] = CreateMonIcon(targetSpecies, SpriteCB_MonIcon, 50 + 32*base_i, 31, 4, personality, FALSE);
...
static void HandlePreEvolutionSpeciesPrint(u8 taskId, u16 preSpecies, u16 species, u8 base_x, u8 base_y, u8 base_y_offset, u8 base_i)
{
    ...
    if (base_i < 3)
    {
        ...
        gTasks[taskId].data[4+base_i] = CreateMonIcon(preSpecies, SpriteCB_MonIcon, 18 + 32*base_i, 31, 4, personality, FALSE); //Create pokemon sprite
        gSprites[gTasks[taskId].data[4+base_i]].oam.priority = 0;
    }
}
...
static void Task_LoadFormsScreen(u8 taskId)
{
    ...
    case 3:
        if (gTasks[taskId].data[1] == 0)
        {
            ...
            FreeMonIconPalettes(); //Free space for new pallete
            LoadMonIconPalettePersonality(species, personality); //Loads pallete for current mon
            gTasks[taskId].data[4] = CreateMonIcon(species, SpriteCB_MonIcon, 18, 31, 4, personality, FALSE); //Create pokemon sprite
            gSprites[gTasks[taskId].data[4]].oam.priority = 0;
        }
        EvoFormsPage_PrintNavigationButtons(); // Navigation buttons
...
static void PrintForms(u8 taskId, u16 species)
{
    ...
    for (i=0; i < 30; i++)
    {
        ...
        else
        {
            ...
            if (times < 7)
                gTasks[taskId].data[4+times] = CreateMonIcon(speciesForm, SpriteCB_MonIcon, 52 + 34*(times-1), 31, 4, personality, FALSE); //Create pokemon sprite
            else if (times < 14)
                gTasks[taskId].data[4+times] = CreateMonIcon(speciesForm, SpriteCB_MonIcon, 18 + 34*(times-7), 70 - y_offset_icons, 4, personality, FALSE); //Create pokemon sprite
