...
static void LoadOverWorld(u8 anim)
{
    ...
    spriteId = AddPseudoObjectEvent(rivalGfxId, SpriteCallbackDummy, 40, 40, 0);
...
static void LoadMonIcon(u8 anim)
{
	...
	for (i = 0; i < gPlayerPartyCount; i++)
	{
		...
		if (anim == 0)
		{
			spriteId = CreateMonIcon(species, SpriteCallbackDummy, 32 * i + 40, 88, 0, personality, 1);
		}else{
			spriteId = CreateMonIcon(species, SpriteCB_MonIcon, 32 * i + 40, 88, 0, personality, 1);
		}
		StartSpriteAnim(&gSprites[spriteId], 0);
	}
}
...
static void SetStdFrame0OnBg(u8 bgId)
{
    TextWindow_SetStdFrame0_WithPal(MAIN_MENU_WINDOW_ERROR, 0x1B1, 0xE0);
    MainMenu_EraseWindow(&sMainMenuWindowTemplates[MAIN_MENU_WINDOW_ERROR]);
}
