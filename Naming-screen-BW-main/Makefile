...
export ARMIPS ?= tools/armips.exe
export GFX ?= tools/gbagfx.exe
export PREPROC ?= tools/preproc.exe
export LD := $(PREFIX)ld
...
export WARNINGFLAGS :=	-Wall -Wno-discarded-array-qualifiers \
	-Wno-int-conversion
export CFLAGS := -g -O2 $(WARNINGFLAGS) -mthumb $(INCLUDES) -mcpu=arm7tdmi \
	-march=armv4t -mno-thumb-interwork -fno-inline -fno-builtin -mlong-calls \
	-fdiagnostics-color -mtune=arm7tdmi -finline
