...
static void StartMenuBW_HideForSave(void)
{
    if (sStartMenuBW == NULL || sStartMenuBW->menuHidden)
        return;

    sStartMenuBW->menuHidden = TRUE;
    HideBg(BG_START_MENU_BACKGROUND);
    HideBg(BG_START_MENU_TEXT);
    StartMenuBW_ClearWindows();
    StartMenuBW_CommitWindows();
    FillBgTilemapBufferRect_Palette0(BG_START_MENU_TEXT, 0, 0, 0, 32, 32);
    CopyBgTilemapBufferToVram(BG_START_MENU_TEXT);
    ResetSpriteData();
    FreeAllSpritePalettes();
    FreeAllWindowBuffers();
...
static void StartMenuBW_RestoreAfterSave(void)
{
    if (sStartMenuBW == NULL || !sStartMenuBW->menuHidden)
        return;
